...
export default function AllPerks() {
  ...
  // ===== useEffect #1: Initial load (get all perks) =====
  useEffect(() => {
    loadAllPerks()
  ...
  useEffect(() => {
    // Debounce so we don’t spam the API while typing/selecting
    const id = setTimeout(() => {
      loadAllPerks()
    }, 350)
  ...
  // ===== useEffect: Build unique merchants list whenever perks change =====
  useEffect(() => {
    const merchants = perks
